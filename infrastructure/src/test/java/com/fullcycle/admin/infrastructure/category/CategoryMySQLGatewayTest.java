...
public class CategoryMySQLGatewayTest {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CategoryMySQLGatewayTest.class);
    ...
    public void givenAValidCategory_whenCallsCreate_shouldReturnANewCategory() {
        ...
        final var actualEntity = categoryRepository.findById(actualCategory.getId().getValue()).orElseThrow();
    ...
    public void givenAPrePersistedCategoryAndValidCategoryId_whenCallsFindById_shouldReturnCategory() {
        ...
        logger.info("Criando nova categoria: {}", aCategory);

        Assertions.assertEquals(0, categoryRepository.count());

        categoryRepository.saveAndFlush(CategoryJpaEntity.from(aCategory));

        logger.info("Categoria salva no banco: {}", aCategory);

        Assertions.assertEquals(1, categoryRepository.count());

        final var savedCategory = categoryRepository.findById(aCategory.getId().getValue()).orElseThrow(() -> new AssertionError("Categoria não encontrada após salvar"));
        logger.info("Categoria salva recuperada: {}", savedCategory);

        logger.info("ID da categoria salva: {}", savedCategory.getId());

        final var actualCategoryOptional = categoryGateway.findByID(aCategory.getId());
        logger.info("Categoria retornada pelo gateway: {}", actualCategoryOptional);

        Assertions.assertTrue(actualCategoryOptional.isPresent(), "Categoria não encontrada no banco de dados");

        final var actualCategory = actualCategoryOptional.get();

        Assertions.assertEquals(1, categoryRepository.count());

        logger.info("isActive: {}", actualCategory.isActive());
        ...
        Assertions.assertEquals(expectedIsActive, actualCategory.isActive());
        Assertions.assertEquals(aCategory.getCreatedAt(), actualCategory.getCreatedAt());
        Assertions.assertFalse(aCategory.getUpdatedAt().isBefore(actualCategory.getUpdatedAt()));
        Assertions.assertEquals(aCategory.getDeletedAt(), actualCategory.getDeletedAt());
        Assertions.assertNull(actualCategory.getDeletedAt());
    }
